...
from utils import mostrar_sidebar, page_header, section_label
...
def mostrar_form_anamnesis(id_paciente, id_contrato, datos_existentes):
    ...
    with st.form("form_anamnesis"):
        ...
        with col2:
            ...
            horas_trab = st.number_input(
                "Horas trabajo/día",
                min_value=0,
                max_value=24,
                value=int(d.get("horas_trabajo") or 8),
            )
            horas_sueno = st.number_input(
                "Horas sueño/día",
                min_value=0,
                max_value=24,
                value=int(d.get("horas_sueno") or 7),
            )
            consumo_agua = st.number_input(
                ...
                max_value=10.0,
                step=0.1,
                value=float(d.get("consumo_agua_litros") or 1.5),
            )
            opciones_e = ["bajo", "moderado", "alto", "muy_alto"]
            idx_e = opciones_e.index(d.get("nivel_estres")) if d.get("nivel_estres") in opciones_e else 1
...
if rol == "paciente":
    # En paciente tomamos el id directamente del usuario
    ...
    if id_paciente and id_paciente in opciones.values():
        nombre_actual = next((k for k, v in opciones.items() if v == id_paciente), nombres[0])
        idx_actual = nombres.index(nombre_actual)
    else:
        idx_actual = 0
        id_paciente = opciones[nombres[0]]

    section_label("Seleccionar paciente")
    sel = st.selectbox(
        "Seleccioná un paciente",
        nombres,
        index=idx_actual,
        key="selector_ficha_paciente",
...
if c:
    ...
    st.progress(
        realizadas / total if total > 0 else 0,
    ...
    if todas_anamnesis:
        ...
        with col_e:
            if len(todas_anamnesis) > 1:
                st.checkbox("Ver historial", key="anam_hist")
    ...
    if historia:
        ...
        try:
            ...
            with col_g1:
                ...
                if len(df_peso) > 0:
                    ch = alt.Chart(df_peso).mark_line(point=True, color="#00DC8E").encode(
            ...
            with col_g2:
                ...
                if len(df_imc) > 0:
                    ch2 = alt.Chart(df_imc).mark_line(point=True, color="#8C52FF").encode(
                        x=alt.X("version:O", title="Sesión"),
            ...
            if len(df_circ) > 0:
                ...
                ch3 = alt.Chart(df_melt).mark_line(point=True, strokeWidth=3).encode(
                    x=alt.X("version:O", title="Sesión"),
                    y=alt.Y("cm:Q", scale=alt.Scale(zero=False), title="cm"),
                    color=alt.Color(
                        "medida:N",
                        scale=alt.Scale(
                            domain=["Cintura", "Cadera", "Brazo"],
        ...
        cols_t = [c_ for c_ in ["Sesión", "Fecha", "Peso", "Talla", "IMC", "Cintura", "Cadera", "Brazo", "Avances"] if c_ in df_tabla.columns]
...
with tab4:
    ...
    planes = run_query(
        ...
        SELECT pl.id_plan, pl.version, pl.titulo, pl.estado,
               ...
               pl.contenido, pl.contenido_json, pl.archivo_url,
               n.nombre || ' ' || n.apellido AS nutricionista
    ...
    if planes:
        activos = [pl for pl in planes if pl["estado"] == "activo"]
        historicos = [pl for pl in planes if pl["estado"] != "activo"]

        def _render_plan_card(plan, active_label=False):
            ...
            with st.container(border=True):
                ...
                with col1:
                    ...
                    badge = "🟢 " if active_label else ""
                    st.markdown(f"{badge}**{titulo}** — {plan['nutricionista']}")
                ...
                with col3:
                    if parsed["kind"] == "structured":
                        pdf_bytes = build_plan_pdf(parsed["data"])
                        st.download_button(
                            "Descargar",
                            data=pdf_bytes,
                            ...
                            key=f"desc_pdf_{plan['id_plan']}",
                        )

                if st.button("Ver / ocultar", key=f"toggle_{plan['id_plan']}", use_container_width=True):
                    st.session_state[f"show_{plan['id_plan']}"] = not st.session_state.get(f"show_{plan['id_plan']}", False)

                if st.session_state.get(f"show_{plan['id_plan']}", False):
                    st.markdown("---")
                    if parsed["kind"] == "structured":
                        show_plan_preview(parsed["data"], height=760)
                    else:
                        st.markdown(parsed["legacy_text"] or "—")

        if activos:
            st.markdown("**Plan activo:**")
            _render_plan_card(activos[0], active_label=True)

        if historicos:
            st.markdown("---")
            st.markdown("**Historial de planes:**")
            for hp in historicos:
                titulo_hist = hp.get("titulo") or f"Plan v{hp['version']}"
                with st.expander(f"{titulo_hist} · v{hp['version']} · {hp['estado']}"):
                    _render_plan_card(hp, active_label=False)

    else:
        st.info("Todavía no hay planes nutricionales cargados.")

    if rol in ("administrador", "nutricionista"):
        st.markdown("---")
        if st.button("Crear nuevo plan", use_container_width=True, type="primary", key="crear_plan_nuevo"):
    ...
    if not c:
        ...
        if sesiones:
            ...
            df_s = df_s.rename(columns={
                ...
                "contador_reprogramaciones": "Reprog.",
            })
            st.dataframe(
                df_s[["", "N°", "Fecha", "Modalidad", "Estado", "Nutricionista", "Reprog."]],
                use_container_width=True,
            )
                    ...
                    if prog:
                        ...
                        if st.button("Marcar como atendida", key="btn_atendida"):
                            run_command(
                                """
                                UPDATE sesiones
                                SET estado = 'atendida', fecha_hora_atencion = NOW()
                                WHERE id_contrato = %s
                                  AND numero_sesion = %s
                                """,
